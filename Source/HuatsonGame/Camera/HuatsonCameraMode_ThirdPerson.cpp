...
#include "HuatsonCameraMode_ThirdPerson.h"
#include "Camera/HuatsonCameraMode.h"
#include "Components/PrimitiveComponent.h"
#include "Camera/HuatsonPenetrationAvoidanceFeeler.h"
#include "Curves/CurveVector.h"
#include "Engine/Canvas.h"
#include "GameFramework/CameraBlockingVolume.h"
#include "HuatsonCameraAssistInterface.h"
#include "GameFramework/Controller.h"
#include "GameFramework/Character.h"
#include "Math/RotationMatrix.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(HuatsonCameraMode_ThirdPerson)

namespace HuatsonCameraMode_ThirdPerson_Statics
{
	static const FName NAME_IgnoreCameraCollision = TEXT("IgnoreCameraCollision");
}

UHuatsonCameraMode_ThirdPerson::UHuatsonCameraMode_ThirdPerson()
{
	TargetOffsetCurve = nullptr;

	PenetrationAvoidanceFeelers.Add(FHuatsonPenetrationAvoidanceFeeler(FRotator(+00.0f, +00.0f, 0.0f), 1.00f, 1.00f, 14.f, 0));
	PenetrationAvoidanceFeelers.Add(FHuatsonPenetrationAvoidanceFeeler(FRotator(+00.0f, +16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FHuatsonPenetrationAvoidanceFeeler(FRotator(+00.0f, -16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FHuatsonPenetrationAvoidanceFeeler(FRotator(+00.0f, +32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FHuatsonPenetrationAvoidanceFeeler(FRotator(+00.0f, -32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FHuatsonPenetrationAvoidanceFeeler(FRotator(+20.0f, +00.0f, 0.0f), 1.00f, 1.00f, 00.f, 4));
	PenetrationAvoidanceFeelers.Add(FHuatsonPenetrationAvoidanceFeeler(FRotator(-20.0f, +00.0f, 0.0f), 0.50f, 0.50f, 00.f, 4));
}

void UHuatsonCameraMode_ThirdPerson::UpdateView(float DeltaTime)
{
...
void UHuatsonCameraMode_ThirdPerson::UpdateForTarget(float DeltaTime)
{
...
void UHuatsonCameraMode_ThirdPerson::DrawDebug(UCanvas* Canvas) const
{
...
void UHuatsonCameraMode_ThirdPerson::UpdatePreventPenetration(float DeltaTime)
{
	...
	IHuatsonCameraAssistInterface* TargetControllerAssist = Cast<IHuatsonCameraAssistInterface>(TargetController);

	IHuatsonCameraAssistInterface* TargetActorAssist = Cast<IHuatsonCameraAssistInterface>(TargetActor);
	...
	IHuatsonCameraAssistInterface* PPActorAssist = OptionalPPTarget.IsSet() ? Cast<IHuatsonCameraAssistInterface>(PPActor) : nullptr;
	...
	if (PPActorRootComponent)
	{
		...
		IHuatsonCameraAssistInterface* AssistArray[] = { TargetControllerAssist, TargetActorAssist, PPActorAssist };

		if (AimLineToDesiredPosBlockedPct < ReportPenetrationPercent)
		{
			for (IHuatsonCameraAssistInterface* Assist : AssistArray)
			{
				if (Assist)
				{
...
void UHuatsonCameraMode_ThirdPerson::PreventCameraPenetration(class AActor const& ViewTarget, FVector const& SafeLoc, FVector& CameraLoc, float const& DeltaTime, float& DistBlockedPct, bool bSingleRayOnly)
{
#if ENABLE_DRAW_DEBUG
	...
	//TODO IHuatsonCameraTarget.GetIgnoredActorsForCameraPentration();
	...
	{
		FHuatsonPenetrationAvoidanceFeeler& Feeler = PenetrationAvoidanceFeelers[RayIdx];
		if (Feeler.FramesUntilNextTrace <= 0)
		{
			...
			if (bHit && HitActor)
			{
				...
				if (HitActor->ActorHasTag(HuatsonCameraMode_ThirdPerson_Statics::NAME_IgnoreCameraCollision))
				{
					bIgnoreHit = true;
...
void UHuatsonCameraMode_ThirdPerson::SetTargetCrouchOffset(FVector NewTargetOffset)
{
...
void UHuatsonCameraMode_ThirdPerson::UpdateCrouchOffset(float DeltaTime)
{
